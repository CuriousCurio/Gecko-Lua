--!nocheck
local ClassInstance = require(script.ClassInstance);
local PUBLIC;
local Class = {
...
PUBLIC = {
	...
	Destroy = function(self) -- Destroys all of the Instances in a class
		...
		for cx:RBXScriptConnection in self.Removing:Iterate() do
			pcall(cx);
		end
...
Class.RBXScriptSignal = ClassInstance.RBXScriptSignal;
Class.RBXMaid = ClassInstance.RBXMaid;
...
function Class.new(table:{})
	local self = setmetatable({
		Instances = {},
		Added = Class.RBXScriptSignal.new(),			-- An instance is added
		Removing = Class.RBXScriptSignal.new(),		-- An instance is being removed
		Destroying = Class.RBXScriptSignal.new(),		-- This Class is being destroyed
